...
public class funcFFT {
    ...
    public static double[] xcorr(int[] recData, int[] rawData, boolean recSmallThanRaw) {
        int length1 = recData.length;
        int length2 = rawData.length;
        int[] recExt = new int[length2];
        for (int i = 0; i < length1; i++) {
            recExt[i] = recData[i];
        }
        Complex[] recFFT = fft(int2complex(recExt));// 有偏移的数据，
        Complex[] rawFFT = fft(int2complex(rawData)); // 原始的无偏移的数据
